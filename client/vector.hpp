...
public:
	...
	friend QTextStream& operator >> (QTextStream &in, vector &rhs) {
		int tmp; in >> tmp;
		rhs.clear();
		for (int i = 0; i < tmp; ++i) {
			T x; in >> x;
	...
	friend QTextStream& operator << (QTextStream &out, const vector &rhs) {
		out << rhs.NowLength << endl;
		for (int i = 0; i < rhs.NowLength; ++i)
			out << rhs.storage[i] << endl;
		return out;
	}
};
